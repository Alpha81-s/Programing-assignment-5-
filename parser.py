...
def parse_expression(ts: TokenStream) -> ASTNode:
    ...
    while ts.peek().tokentype != TokenType.EOF:
        ...
        if tok.tokentype == TokenType.INTLIT:
            ...
            next = ts.peek()
            if (next.tokentype in operatortypes) or (next.tokentype == TokenType.RPAREN) or (
                    next.tokentype == TokenType.EOF):
        ...
        if tok.tokentype == TokenType.VARREF:
            ...
            pp = ts.peek()  # parse peek
            if (pp.tokentype in operatortypes) or (pp.tokentype == TokenType.RPAREN) or (pp.tokentype == TokenType.EOF):
        ...
        if tok.tokentype == TokenType.LPAREN:
            ...
            opstack.append(tok)
            pp = ts.peek()  # parse peek
            if pp.tokentype not in {TokenType.LPAREN, TokenType.INTLIT, TokenType.VARREF}:
        ...
        if tok.tokentype in operatortypes:
            ...
            while len(opstack) > 0 and opstack[-1].tokentype in operatortypes:
                ...
                if top_prec > inc_prec:
                    reduce(opstack, valstack)
                    #   reduce(opstack, valstack)
                elif top_prec == inc_prec and leftassoc[incoming.tokentype]:  # else
                    reduce(opstack, valstack)
                else:  # break
                    break
...
def reduce(opstack: list, valstack: list) -> None:
    ...
    if len(opstack) == 0:
        # If not, raise error
        raise ParseError("Operator stack underflow in reduce")
    # Check overflow
    if len(valstack) < 2:
        op_end = opstack[-1]
        raise ParseError(f"Expected two operands for operator {op_end.tokentype}")
    ...
    valstack.append(BinOpNode(op_tok.tokentype, lhs, rhs))
